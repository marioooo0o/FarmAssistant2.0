...
export default {
    ...
    setup(props, { emit }) {
        ...
        function checkForm() {
            ...
            if (practiseName.value && practiseDate.value && practiseFields.value.length !== 0 && practiseFields.value.length !== 0 && practiseWater.value) {
                return true;
            }
        ...
        async function submitForm() {
            ...
            if (checkForm()) {
                const formData = {
                    ...
                    products: practiseProducts.value.map((product) => ({"id": product.id, "quantity": product.quantity})),
                }
                    ...
                    for (let e in response.errors) {
                        switch (e) {
                            ...
                            case 'start_date':
                                ...
                                break;
                            case 'products':
                                errors.practiseProducts.push(...response.errors[e]);
                                break;
                            case 'fields':
